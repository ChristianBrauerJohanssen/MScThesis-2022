...
####################
@njit(parallel=True)
def postdecision_compute_v_bar_q_old(t,sol,par):
    ...
    for i_w in prange(par.Nw):
        ...
        for i_h in prange(par.Nh):
            ...
            for i_x in range(par.Nx):                        
                ...
                d_prime_high = np.fmin(par.omega_ltv*par.q*h,par.omega_dti*y) 
                grid_d_prime = np.linspace(0,d_prime_high,par.Nd)
...
def obj_buy(d_prime,h_buy,m_gross,inv_v_stay_slice,grid_m,grid_d_prime,par):
    ...
    # c. add pecuniary cost of moving
    #v_buy = -1/linear_interp.interp_2d(grid_m,grid_d_prime,inv_v_stay_slice,m_net,d_prime)-par.kappa    
    #m_net = trans.m_to_mnet_buy(m,h,d,d_prime,hbuy,par)
    # c. value-of-choice
    return linear_interp.interp_2d(grid_m,grid_d_prime,inv_v_stay_slice,m_net,d_prime)
                            ...
                            for i_hb in range(par.Nh):
                                ...
                                for i_dp in range(len(grid_d_prime)): 
                                    for Tda in range(max(grid_Tda)):
                                        ...
                                        # evaluate choice
                                        inv_v_buy_new = obj_buy(grid_d_prime[i_dp],h_buy_now,m_gross,
...
@njit
def obj_buy_fast(d_prime,h_buy,m_gross,inv_v_stay_slice,grid_m,grid_d_prime,par):
    """ interpolate on stayer solution for buyers """
    ...
    for i_w in prange(par.Nw):
        ...
        for i_x in prange(par.Nx):               
            ...
            for i_hb in range(par.Nh):
                ...
                d_prime_high = np.fmin(par.omega_ltv*par.q*h_buy_now,par.omega_dti*y)   
                grid_d_prime = np.linspace(0,d_prime_high,par.Nd)                       
                ...
                for i_dp in range(len(grid_d_prime)): 
                    for Tda in range(max(grid_Tda)):
                        # evaluate choice
                        inv_v_buy_new = obj_buy_fast(grid_d_prime[i_dp],h_buy_now,m_gross,
                ...
                h_buy[i_w,i_x] = hbuy_best

                ## buyer's net cash on hand equation
                loan = 0 
                if d_prime_best > 0:
                    loan = 1
                m_net = m_gross-loan*par.Cf_ref+(1-par.Cp_ref)*d_prime_best-(1+par.C_buy)*par.q*hbuy_best
...
def force_default(y_plus,i_shock,t,sol,par):
    ...
    for i_ht in range(par.Nhtilde):
        ...
        inv_v_rent = np.fmax(
            linear_interp.interp_1d(par.grid_w,
        ...
        if inv_v_def_temp > inv_v_default:
            ...
            inv_marg_u_def = linear_interp.interp_1d(
                                par.grid_w,
